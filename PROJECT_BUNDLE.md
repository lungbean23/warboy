...
DO_REGEN=0
PUSH_REMOTE="origin"
REMOTE_URL="${REMOTE_URL:-}"   # optional env var to add remote if missing
COMMIT_MSG=""
...
Environment:
  REMOTE_URL=<url>    If --remote (default origin) does not exist, add it with this URL.
...
ensure_remote() {
  ...
  if [[ -n "$REMOTE_URL" ]]; then
    echo "Remote '$PUSH_REMOTE' missing; adding it with REMOTE_URL=$REMOTE_URL"
    git remote add "$PUSH_REMOTE" "$REMOTE_URL"
  else
    echo "ERROR: remote '$PUSH_REMOTE' missing and REMOTE_URL not provided."
    echo "Provide REMOTE_URL=git@github.com:<user>/<repo>.git (or https URL) and re-run."
    exit 1
  fi
}
...
push_with_upstream() {
  ...
  # If upstream exists, plain push; else push -u to set upstream.
  if git rev-parse --abbrev-ref --symbolic-full-name @{u} >/dev/null 2>&1; then
    git push "$PUSH_REMOTE" "$branch"
  else
    echo "No upstream for $branch; pushing with -u $PUSH_REMOTE $branch"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --remote)
      [[ $# -ge 2 ]] || { echo "ERROR: --remote requires a name"; exit 1; }
...
# ---- Commit or exit if nothing staged
...
# ---- Push (auto-add remote and set upstream if needed)
